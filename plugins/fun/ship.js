...
export default {
  ...
  async run({ sock, from, sender, msg, reply, react }) {
    ...
    if (!userB) {
      return await reply({
        text: `🌸 Mencioná a alguien (o respondé a su mensaje) para hacer el ship.\n\n` +
          `✨ *Uso:* .ship @usuario`
      })
    }

    if (userA === userB) {
      return await reply({ text: `⚠️ No podés hacerte ship con vos mismo, xd.` })
    }
