export default {
  ...
  async run({ sock, from, msg, reply }) {
    try {
      ...
      // Ejecutar la apertura del grupo directamente
      await sock.groupSettingUpdate(from, 'not_announcement')
      await reply({ text: "꒰ 𑁍 ꒱ E𝗅 gꭇᥙ⍴o ⍺ 𝗌іძo ⍺ᑲiᧉꭇƚo.\n> ¡Ahora todos los miembros pueden enviar mensajes!" })
      
    } catch (e) {
      console.error('Error en abrir:', e)
      await reply({ text: `❌ Hubo un error al abrir el grupo. Asegúrate de que el bot sea administrador.\n> Detalles: ${e.message}` })
    }
  }
}
